...
const AgregarOrden = () => {
    ...
    const handleItemChange = (e) => {
        ...
        // Actualizar precio unitario cuando se selecciona producto
        if (name === 'productId') {
    ...
    const calcularIVA = () => {
        return ordenData.totalAmount * 0.16; // 16% IVA
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(error.response?.data?.title || "Error al crear la orden");
        } finally {
            setLoading(false);
        }
    };
